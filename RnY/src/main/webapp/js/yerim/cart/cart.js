...
// 수량 조절 버튼에 대한 이벤트 리스너 추가
    document.querySelectorAll('.decrease, .increase').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            // 수량을 증가할 번호 
            let cartNo = row.querySelector('input[name=cartNo]').value;
            // 수량을 입력할 위치 
            let orderQuantity = document.querySelector('#cartForm>#quantity_' + cartNo);
            // 상품 가격을 가져옴
            ...
            if (button.classList.contains('increase')) {
                ...
                quantity = Math.max(1, quantity - 1); // 수량을 1 이상으로 감소
            }
